...
| G11 | Integration -> Audit **[RETURN: Codex -> Claude]** | Integration Owner | Codex -> Claude | **Gate Blocked** | Held by G12 whole-repo systematic sweep (5th kickback) | 2026-06-04 | Sweep found release-blocking defects in code never touched by rounds 1–4: IDOR (BLK-G12-015/016), unreliable identity-authz audit (017), missing optimistic concurrency (018), + more. Rework #5: `delivery/G12-rework-5.md`. |
| G12 | Audit -> Release/Rework | Audit | Claude | **Gate Blocked (5th REWORK — systematic sweep)** | Audit (independent): 1st audit + re-audit + final re-audit + spot re-audit + whole-repo systematic sweep (7 dimensions) | 2026-06-04 | Whole-repo systematic sweep (NOT scoped to recent changes) found BLK-G12-015..025: 2 IDOR BLOCKERs, 7 MAJOR, 2 batch MINOR/LOW — in areas never audited by the narrow prior rounds. Codex constraints: no downgrade AND no over-reach. After remediation Claude re-runs the FULL systematic sweep before any pass. Decision: `archive/reviews/g12-audit/g12-systematic-sweep-2026-06-04.md`. |
