...
| G11 | Integration -> Audit **[RETURN: Codex -> Claude]** | Integration Owner | Codex -> Claude | Gate Blocked | | 2026-06-03 | BLK-G11-002: TASK-039 has deployed HTTPS/IP-certificate evidence on `srv-volcengine-sh-01`; closeout attempt confirmed healthy host/container/TLS posture but no local Volcengine CLI/API credentials or metadata source to export security-group rules. Still lacks Volcengine cloud security-group inbound rule evidence and owner/security review of pre-existing public `8642` Hermes exposure. |
