...
| BLK-G12-006 | MAJOR | Release evidence not independently verifiable: TLS facts hand-typed (no openssl/curl/certbot capture), restore rehearsal is operator-authored counts (no transcript/checksum output), operator-access conditions 2 & 3 unevidenced (root deploy), no external negative probe for 8080/5432. G12 rework captured curl/openssl/certbot/negative-probe/restore/operator-access transcripts and hardened SSH password/root policy. | infrastructure-ops | G12 release | ACC-017 (P0); operator-access review conditions | Resolved | 2026-06-03 |
| BLK-G12-007 | MAJOR | CRM instance shares the `Default` security group (all-protocol self-referential allow) instead of a dedicated least-exposure group. CRM ENI is now bound only to dedicated security group `sg-366ptx1bxp9ts1e710babmc8y`, whose public TCP ingress is limited to `22`, `80`, and `443`. | infrastructure-ops | G12 release | ACC-017; deployment-notes Network Exposure | Resolved | 2026-06-03 |
| BLK-G12-008 | MINOR | Test-traceability + hardening: untagged P0 tests (TEST-ABUSE-MUTATE-001, TEST-AUTHZ-SCOPE-005, TEST-PERM-CRUD-ADMIN/MGR/SALES-001, TEST-OWNER-TRANSFER-002/004); `work` S2S verifier omits the 5-min lifetime cap; `AUTHZ_VERSION_STALE` is dead code; stray screenshot at repo root. Rework added explicit test IDs, enforced work S2S max 5-minute lifetime, returned `AUTHZ_VERSION_STALE` on stale authz versions, and removed the root screenshot. | backend-engineer | G12 close (non-release-blocking) | traceability; minor security hardening | Resolved | 2026-06-03 |
...
| 2026-06-03 | BLK-G12-006 | Captured independently verifiable release transcripts: external HTTPS `curl -I -v` with HTTP/2 200 and HSTS, HTTP→HTTPS redirect transcript, `openssl s_client` certificate transcript, `certbot certificates` plus `certbot renew --dry-run`, external negative probes for `8080` and `5432` using source interface `192.168.0.107`, backup checksum/decrypt/catalog restore transcript, and operator-access transcript. Hardened SSH with `PasswordAuthentication no`, `KbdInteractiveAuthentication no`, and `PermitRootLogin prohibit-password`; `TEST-RELEASE-EVIDENCE-001` passed. | `docs/release/evidence/tls-curl-https-2026-06-03.txt`; `docs/release/evidence/tls-openssl-2026-06-03.txt`; `docs/release/evidence/tls-curl-redirect-2026-06-03.txt`; `docs/release/evidence/certbot-renew-dry-run-2026-06-03.txt`; `docs/release/evidence/external-negative-probes-2026-06-03.txt`; `docs/release/evidence/backup-restore-transcript-2026-06-03.txt`; `docs/release/evidence/operator-access-transcript-2026-06-03.txt`; `scripts/test_release_evidence_transcripts.sh` |
