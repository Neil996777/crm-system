...
| BLK-G11-002 | TASK-039 has server-side HTTPS/TLS, health, redirect, service health, monitoring threshold, renewal, and operator evidence, but production closure still lacks Volcengine cloud security-group inbound rule evidence. Infrastructure Ops read-only check and Codex closeout attempt on 2026-06-03 confirmed host-level CRM gateway `127.0.0.1:8080`, PostgreSQL without a host port mapping, healthy CRM containers, active TLS renewal, and no local Volcengine CLI/API credentials or metadata source that can export provider rules; however, ACC-017 requires provider security-group evidence, and the pre-existing public `8642` Hermes exposure remains a non-CRM port with owner `TBD` in infrastructure registers that needs owner/security review. | Infrastructure Ops + Security Compliance | G11 handoff to G12; TASK-039 | ACC-017 (P0), ARCH-ACC-013, ARCH-ACC-014, TEST-DEPLOY-SMOKE-001/002 | Open | 2026-06-03 |
