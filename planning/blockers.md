...
| BLK-G12-006 | MAJOR | Release evidence not independently verifiable. **Re-audit: PARTIAL** — TLS/redirect/cert/certbot/restore/operator-access are now REAL captured transcripts (good), but residuals remain: restore-rehearsal counts contradict the attached catalog output (JSON 9 vs `\dn`/`\du` 10); `crm-deploy` and `crm-ops` share the **same SSH key fingerprint**; no `sshd -T` proof the hardening drop-in is effective; negative probe ran from LAN, not an external edge. | infrastructure-ops | ACC-017 (P0); operator-access review conditions | **Open (partial)** | 2026-06-03 |
...
| BLK-G12-009 | MAJOR | Evidence-integrity gap: `scripts/test_security_group_evidence.py` validates only JSON shape and accepts a read-only snapshot as "success" (this let the 20:29 read-only file pass as if it proved a mutation). Harden the checker to require the four mutating-call RequestIds (or treat read-only exports as verification-only, never as remediation proof). | infrastructure-ops | evidence integrity | **Open** | 2026-06-03 |
