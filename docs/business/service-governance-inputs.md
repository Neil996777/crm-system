...
| BSG-FLOW-005 | Contract signing to Won; payment tracked post-sale | User signs contract and closes opportunity; payment recorded as post-sale follow-up | CAP-004, CAP-005, CAP-008 | Opportunity becomes Won when the related contract is Signed (DEC-017); payment state reflects actual payments and is decoupled from Won (DEC-019). | Zero, negative, overpayment, Won without a Signed contract, or missing lost reason are rejected. | ACC-011, ACC-013, ACC-014 |
...
| BIZ-EVT-011 | Payment recorded | Actual payment succeeds | Payment status recalculation; history/log (payment is decoupled from Won, DEC-019). | ACC-011, ACC-013, ACC-014, ACC-022 |
...
| Opportunity | CAP-004 | Required customer, owner, stage, amount, expected close date; Won/Lost terminal (DEC-020: `status` removed). | Unauthorized records excluded from rows and aggregates. | CAP-004, CAP-009 |
| Quote | CAP-005 | Exactly one quote per opportunity (DEC-018); expired quote cannot create contract. | Quote amount and status are restricted by record scope. | CAP-005 |
