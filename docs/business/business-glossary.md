...
| Partial Payment | Payment state where cumulative paid amount is greater than zero and less than contract amount. | PRD-011 | Supported in the committed scope |
...
| Reminder | In-app notification for due/overdue tasks, Pending Signature contracts past expected signed date, or due/overdue payments. | PRD-021 | In-app only for the committed release |
...
| Hard Delete | Permanent deletion of core CRM records. | PRD-002 | Not allowed in the committed scope |
...
| CSV Import | The committed import format for authorized bulk data entry. | PRD-020 | Validates row by row |
| CSV Export | The committed export format for authorized CRM records. | PRD-020 | Sales cannot export |
