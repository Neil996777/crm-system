...
func TestOutboxDispatcherDeliversRetriesAndDedupesAuditHistoryEvents(t *testing.T) {
	...
	if err := outbox.Append(ctx, ContractStatusChanged, "contract_dispatch_1", map[string]any{
		"actorId":       "sales-1",
		"actorRole":     "Sales",
		"actorDisplay":  "Sales One",
		"correlationId": "corr-commercial-dispatch",
		"toStatus":      "Signed",
		"ownerId":       "sales-1",
		"amount":        "10000.00",
	}); err != nil {
	...
	eventUID := readOutboxID(t, db, "contract_dispatch_1")
	...
	if err := outbox.DispatchOnce(ctx, DispatchConfig{
		...
		t.Fatal("TEST-HISTORY-DISPATCH-RETRY-001 expected failed audit append to keep commercial event unpublished")
	}
	if failures != 1 {
	...
	okServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requireAuditRequest(t, r, "commercial", "sales-1", "Sales", "corr-commercial-dispatch")
	...
	if received["eventUid"] != eventUID || received["eventId"] != "EVT-CONTRACT-SIGNED" {
		t.Fatalf("TEST-HISTORY-DISPATCH-RETRY-001 expected commercial signed event uid/id, got %#v", received)
	}

	if _, err := db.Exec(`UPDATE commercial.outbox_events SET published_at = NULL WHERE id = $1`, eventUID); err != nil {
...
func readOutboxID(t *testing.T, db *sql.DB, aggregateID string) string {
	t.Helper()
	var eventUID string
	if err := db.QueryRow(`SELECT id FROM commercial.outbox_events WHERE aggregate_id = $1`, aggregateID).Scan(&eventUID); err != nil {
...
func requirePublishedState(t *testing.T, db *sql.DB, eventUID string, published bool) {
	...
	if err := db.QueryRow(`SELECT published_at IS NOT NULL FROM commercial.outbox_events WHERE id = $1`, eventUID).Scan(&isPublished); err != nil {
