...
func (r *DuplicateRepo) Check(ctx context.Context, actorID, actorRole string, candidate domain.DuplicateCandidate) (domain.DuplicateCheckResult, error) {
	...
	_, err = r.db.ExecContext(ctx, `
		INSERT INTO lead.duplicate_warning_tokens (token, target_type, normalized_signature, actor_user_id, expires_at)
		VALUES ($1, $2, $3, $4, $5)
...
func (r *DuplicateRepo) ConsumeToken(ctx context.Context, token, targetType, actorID, signature string) error {
	...
	err := r.db.QueryRowContext(ctx, `
		...
		FROM lead.duplicate_warning_tokens
	...
	result, err := r.db.ExecContext(ctx, `
		UPDATE lead.duplicate_warning_tokens
		SET used_at = now()
...
func (r *DuplicateRepo) findMatches(ctx context.Context, actorID, actorRole string, candidate domain.DuplicateCandidate) ([]domain.DuplicateMatch, []string, error) {
	...
	if company := domain.NormalizeCompanyName(candidate.CompanyName); company != "" {
		count, err := r.countLeadsByCompany(ctx, actorID, actorRole, company)
		if err != nil {
			return nil, nil, err
		}
		if count > 0 {
			matches = append(matches, safeMatch("lead", "LEAD_COMPANY_MATCH"))
			rules = append(rules, "LEAD_COMPANY_MATCH")
		}
	}
	if email := domain.NormalizeEmail(candidate.Email); email != "" {
		count, err := r.countLeadsByEmail(ctx, actorID, actorRole, email)
		if err != nil {
			return nil, nil, err
		}
		if count > 0 {
			matches = append(matches, safeMatch("lead", "LEAD_EMAIL_MATCH"))
			rules = append(rules, "LEAD_EMAIL_MATCH")
		}
	}
	if phone := domain.NormalizePhone(candidate.Phone); phone != "" {
		count, err := r.countLeadsByPhone(ctx, actorID, actorRole, phone)
		if err != nil {
			return nil, nil, err
		}
		if count > 0 {
			matches = append(matches, safeMatch("lead", "LEAD_PHONE_MATCH"))
			rules = append(rules, "LEAD_PHONE_MATCH")
		}
	}
	return matches, rules, nil
}

func (r *DuplicateRepo) countLeadsByCompany(ctx context.Context, actorID, actorRole, normalizedCompany string) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `
		SELECT count(*)
		FROM lead.leads
...
func (r *DuplicateRepo) countLeadsByEmail(ctx context.Context, actorID, actorRole, normalizedEmail string) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `
		SELECT count(*)
		FROM lead.leads
...
func (r *DuplicateRepo) countLeadsByPhone(ctx context.Context, actorID, actorRole, normalizedPhone string) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `
		SELECT count(*)
		FROM lead.leads
		WHERE regexp_replace(phone, '\D', '', 'g') IN ($1, '86' || $1)
		  AND ($2 <> 'Sales' OR owner_id = $3)
