...
func TestOutboxDispatcherDeliversRetriesAndDedupesAuditHistoryEvents(t *testing.T) {
	...
	if err := outbox.Append(ctx, OpenWorkTransferred, "task_dispatch_1", map[string]any{
		"actorId":       "account",
		"actorRole":     "System",
		"actorDisplay":  "Account Service",
		"correlationId": "corr-work-dispatch",
		"fromOwnerId":   "sales-1",
		"toOwnerId":     "sales-2",
	}); err != nil {
		t.Fatalf("append outbox event: %v", err)
	}
	eventUID := readOutboxID(t, db, "task_dispatch_1")
	...
	if err := outbox.DispatchOnce(ctx, DispatchConfig{
		...
		t.Fatal("TEST-HISTORY-DISPATCH-RETRY-001 expected failed audit append to keep work event unpublished")
	}
	if failures != 1 {
	...
	okServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requireAuditRequest(t, r, "work", "account", "System", "corr-work-dispatch")
	...
	if received["eventUid"] != eventUID || received["eventId"] != "EVT-OWNER-CHANGED" {
		t.Fatalf("TEST-HISTORY-DISPATCH-RETRY-001 expected work owner-change event uid/id, got %#v", received)
	}

	if _, err := db.Exec(`UPDATE work.outbox_events SET published_at = NULL WHERE id = $1`, eventUID); err != nil {
...
func readOutboxID(t *testing.T, db *sql.DB, aggregateID string) string {
	t.Helper()
	var eventUID string
	if err := db.QueryRow(`SELECT id FROM work.outbox_events WHERE aggregate_id = $1`, aggregateID).Scan(&eventUID); err != nil {
...
func requirePublishedState(t *testing.T, db *sql.DB, eventUID string, published bool) {
	...
	if err := db.QueryRow(`SELECT published_at IS NOT NULL FROM work.outbox_events WHERE id = $1`, eventUID).Scan(&isPublished); err != nil {
